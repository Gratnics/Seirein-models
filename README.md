...
A curated catalog of image generation models, built and maintained as part of [Seirein](https://github.com/Gratnics/Seirein) by the same creator.

This repository tracks image generation models across providers and APIs — resolution limits, input handling, configurable settings, output support, and pricing — as structured, sourced data. It exists to give Seirein (and anyone else) a single, up-to-date reference for choosing and integrating image models.
...
- **`models/<Provider>/<model>.toml`** — one TOML file per model, grouped by provider. This is the source of truth; edit these files to add or update a model.
- **`catalog.json`** — every model TOML file combined into a single JSON array, generated from `models/`. Regenerate it after any change under `models/`.
- **`scripts/build_catalog.py`** — the build script that combines `models/*/*.toml` into `catalog.json`.

## Providers covered

Black Forest Labs, ByteDance, Google, Ideogram, Luma AI, OpenAI, Stability AI, Together AI, xAI, and OpenRouter (as an aggregator covering additional providers such as Krea, Microsoft, Qwen, Recraft, and Sourceful).

## Model entry schema

Each model TOML file has the following sections:
...
Example (`models/OpenAI/gpt-image-2.toml`, abridged — `...` marks values shortened for this README; the real file lists every source URL in full):
...
This rebuilds `catalog.json` from scratch out of the current `models/*/*.toml` files, so added, edited, or removed models are reflected automatically. Options:
...
1. Add or edit a `models/<Provider>/<model-name>.toml` file following the schema above. The `[model].provider` value must match its parent folder name.
